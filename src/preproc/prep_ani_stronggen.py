...
DATA_DIR = 'dat/raw/anime/'


# In[4]:


OUT_DATA_DIR = 'dat/proc/an_sg/'
...
raw_data = pd.read_csv(raw_file_path,usecols=['user_id', 'anime_id', 'rating'])
test_data = pd.read_csv(test_file_path,usecols=['user_id', 'anime_id', 'rating'])
...
user_activity = get_count(raw_data, 'user_id')
...
vad_plays_tr, vad_plays_te = split_train_test_proportion(vad_plays, 'user_id', test_prop=0.5, random_seed=13579)
...
test_plays_tr, test_plays_te = split_train_test_proportion(test_plays, 'user_id', test_prop=0.5, random_seed=13579)


# In[22]:


rand_test_plays = test_data.loc[raw_data['user_id'].isin(te_users)]
rand_test_plays = test_plays.loc[test_plays['anime_id'].isin(unique_sid)]


# In[23]:


rand_test_plays_tr, rand_test_plays_te = split_train_test_proportion(rand_test_plays, 'user_id', test_prop=0.5, random_seed=13579)
...
def numerize(tp):
    ...
    sid = list(map(lambda x: song2id[x], tp['anime_id']))
    tp = tp.copy()
    tp['uid'] = uid
    tp['sid'] = sid
