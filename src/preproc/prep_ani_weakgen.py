...
tr_vd_data = pd.read_csv(raw_file_path,usecols=['user_id', 'anime_id', 'rating'])
...
user_activity = get_count(tr_vd_data, 'user_id')
item_popularity = get_count(tr_vd_data, 'anime_id')


# In[11]:

combined_data = pd.concat([tr_vd_data, test_data])
unique_uid = combined_data['user_id'].unique()
unique_sid = combined_data['anime_id'].unique()
...
test_data = test_data.loc[test_data['user_id'].isin(unique_uid)]
test_data = test_data.loc[test_data['anime_id'].isin(unique_sid)]
